...
function previewSummary(project: Project): string {
  ...
  if (!preview) return ''
  if (preview.status === 'running' || preview.status === 'pending') return '正在读取仓库并计算清理预览，尚无结果'
  if (preview.status === 'skipped') return '本次预览已跳过：同一项目有其他任务在执行'
          ...
          <div v-else class="card-list">
            <article v-for="project in group.projects" :key="project.id" class="project-card" :class="{ 'project-disabled': !project.enabled }">
              <div class="project-top">
                ...
                <div class="project-actions"><span class="status-pill" :class="project.enabled ? 'online' : 'neutral'">{{ project.enabled ? '已启用' : '已暂停' }}</span><button type="button" class="ghost compact" :disabled="loading" @click="emit('edit', project)">编辑</button><button type="button" class="primary compact" :disabled="loading || agentWorkDisabled || !project.enabled || queuedProjectIds.has(project.id)" @click="emit('run', project)">{{ queuedProjectIds.has(project.id) ? '已排队 ✓' : '立即备份' }}</button></div>
              </div>
              ...
              <div class="rpo-strip" :class="healthOf(project)?.status || 'unknown'">
                ...
                <div><small>保护状态</small><strong>{{ projectHealthLabel(healthOf(project)) }}</strong></div>
                <span>{{ healthSummary(project) }}</span>
              </div>
              <div class="schedule-overview">
                <div><small>执行计划</small><strong>{{ cronDescription(project.schedule.cron) }}</strong><span>{{ project.schedule.timezone }}<template v-if="project.schedule.jitter_seconds"> · 最多延后 {{ formatTimeBudget(project.schedule.jitter_seconds) }}</template></span></div>
                <div class="next-run"><small>下次计划</small><strong>{{ formatNextRun(project) }}</strong><span>最长运行 {{ formatTimeBudget(project.schedule.max_runtime_seconds) }}</span></div>
              </div>
              <details class="project-details">
              <summary>策略与更多操作 <span>配置版本 {{ project.revision }}</span></summary>
              ...
              <div class="maintenance-strip"><small>维护安排</small><strong>{{ maintenanceSummary(project) }}</strong></div>
              <div class="project-secondary-actions"><button type="button" class="ghost compact" :disabled="loading" @click="emit('toggle', project)">{{ project.enabled ? '暂停' : '恢复' }}</button><button v-if="project.policy?.retention.enabled" type="button" class="ghost compact" :disabled="loading || agentWorkDisabled || !project.enabled || queuedPreviewProjectIds.has(project.id)" @click="emit('preview', project)">{{ queuedPreviewProjectIds.has(project.id) ? '预览排队中' : '清理预览' }}</button><button type="button" class="text-button danger-text" :disabled="loading" @click="emit('archive', project)">归档项目</button></div>
              </details>
