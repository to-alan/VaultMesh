...
- Agent 一键安装：控制台注册卡片直接给出 `install-agent` 一条命令（下载/校验二进制 → systemd → 注册 → 清除令牌），Agent 二进制支持 `--version`；支持 `VAULTMESH_AGENT_VERSION=edge` 从 GHCR 镜像提取跟踪 main 的版本
- `uninstall-agent` 卸载模式：停止服务、清理二进制/配置/设备身份（保留恢复测试产物）；重装时自动重置旧注册，修复"重新执行安装不起作用"（state.json 绑定旧身份导致新令牌失效）
