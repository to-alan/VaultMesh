...
| 管理员安全 | 单管理员账号、密码变更、TOTP、恢复码、WebAuthn 通行密钥、会话撤销、进程内失败渐进限速、持久化认证审计 | 持久化/多副本限速、密码找回、多管理员/RBAC |
| Agent | 一次性注册、设备凭据、配置版本、原子 JSON 本地状态、离线调度、Outbox | 设备凭据轮换/吊销界面、签名发布、自动升级、SQLite 状态迁移 |
...
| 可观测性 | 运行统计、Agent 在线状态、计划倒计时、RPO 迟到/超时、备份失败/RPO 告警、9 类自定义通知渠道、按项目路由、去重、重复提醒、恢复通知、持久化重试、操作审计 | Agent 离线/仓库 Check 失败告警、静默时段、多级升级、审计字段差异/外部归档 |
...
- 密码登录和 TOTP 已实现有界的进程内失败限速，生产反向代理仍需提供持久化或多副本共享限速；注册、Secret 验证和恢复等高风险入口也应按威胁模型独立限速；
