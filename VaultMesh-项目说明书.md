...
| 项目治理 | 公开源码、CI、Compose/systemd、安装和运维文档 | 尚未选择 `LICENSE`；公开可见不等于已授予使用、修改和分发权利 |
...
VaultMesh 是一个公开源码、自托管、多服务器的备份控制平台；在项目所有者明确选择许可证后，目标是作为开源项目发布。它负责：
...
1. 许可证和商业边界。GitHub 官方说明指出，没有许可证时默认版权规则仍然适用；公开仓库不能因此直接宣称为开源发行版；
...
9. GitHub Repository Licensing：<https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/licensing-a-repository>

---

## 21. 最终项目说明

VaultMesh 是一个面向 Linux VPS、Homelab 和小型技术团队的公开源码、自托管备份控制平台。平台通过轻量 Agent 管理多服务器的声明式备份策略，使用 Restic 将客户端加密的数据从源服务器直接写入用户自己的存储，并集中管理运行状态、计划、快照、保留、校验和受控恢复。RPO 违约计算、通知和自动恢复演练是明确的后续能力，不属于当前版本承诺。
