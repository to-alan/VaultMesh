...
> 文档版本：0.5（当前实现基线、非商业许可、工程评审与路线图）
> 更新日期：2026-07-15
...
| 管理员安全 | 单管理员账号、密码变更、TOTP、恢复码、WebAuthn 通行密钥、会话撤销 | 登录审计、应用内限速、密码找回、多管理员/RBAC |
...
| 调度与运行 | 5 段 Cron、IANA 时区、抖动、超时、仓库串行、立即执行、幂等 Run | 错过任务后补跑；当前只支持 `missed_run_policy=skip` |
...
| 可观测性 | 运行统计、失败状态、Agent 在线状态、计划视图、下次任务倒计时 | RPO 违约引擎、通知渠道、告警去重/恢复通知、完整审计日志 |
...
当前阶段判断：阶段 0 的代码闭环已经完成，真实云存储和恢复演练仍需持续执行；阶段 1 正在进行，RPO、通知、审计和发布工程是主要缺口。
...
VaultMesh 是一个面向 Linux VPS、Homelab 和小型技术团队的公开源码、自托管备份控制平台，采用 PolyForm Noncommercial 1.0.0 供非商业用途使用。平台通过轻量 Agent 管理多服务器的声明式备份策略，使用 Restic 将客户端加密的数据从源服务器直接写入用户自己的存储，并集中管理运行状态、计划、快照、保留、校验和受控恢复。RPO 违约计算、通知和自动恢复演练是明确的后续能力，不属于当前版本承诺。
...
当前实现聚焦 Linux、Restic、文件、Docker 挂载、MySQL/PostgreSQL 逻辑导出和多类 Restic 仓库，已经打通“配置—执行—上报—快照—保留/校验—隔离恢复”的纵向链路。下一阶段优先完成真实 Provider 集成测试、RPO 与通知、控制面灾备、恢复演练和发布安全；Redis、数据库自动恢复、不可变存储增强、多管理员权限和多备份引擎继续按阶段评估。
