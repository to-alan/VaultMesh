...
> 文档版本：0.8（当前实现基线、非商业许可、工程评审与路线图）
> 更新日期：2026-07-15
> 文档状态：0.2 节描述已验证实现，其余章节包含目标设计；目标能力不得被表述为已经交付
...
建议继续投入，但项目已经不再处于纯构想阶段。当前代码完成了单台至多台 Agent 的配置同步、本地调度、文件/数据库/Docker 来源准备、Restic 上传、离线上报、快照索引、保留/检查和隔离恢复等纵向链路。下一阶段的重点应从“增加表单和类型”转向可靠性、安全运营、真实 Provider 集成测试和恢复演练。

当前版本仍是 1.0 之前的工程基线。它可以用于本地验证和独立测试仓库，不应在没有并行备份、控制面灾备和真实恢复验收的情况下直接替换生产备份方案。

不建议在首版同时承诺 Redis、SSH Storage、Borg、Kopia、自动恢复整个 Docker 应用、多租户 RBAC、高可用控制面和恢复测试沙箱。这些能力会显著放大测试矩阵，并掩盖核心链路的可靠性问题。

### 0.2 当前实现基线与缺口

下表以 2026-07-15 的仓库代码和自动化测试为准。后续章节是产品与架构目标，只有本表“已实现”项可以作为当前功能承诺。

| 领域 | 当前已实现 | 尚未完成或仍需真实环境验证 |
|---|---|---|
| 控制面 | Go API、PostgreSQL、自动迁移、Vue 3 独立 Web、精确 Origin CORS | 多副本会话、升级兼容矩阵、正式版本与回滚自动化 |
| 管理员安全 | 单管理员账号、密码变更、TOTP、恢复码、WebAuthn 通行密钥、会话撤销、登录与敏感验证渐进限速、TOTP 初始化会话绑定、持久化认证审计 | 持久化/多副本限速、密码找回、多管理员/RBAC |
| Agent | 一次性注册、设备凭据、配置版本、原子 JSON 本地状态、离线调度、失败可回滚 Outbox、禁止控制面重定向和 URL 凭据 | 设备凭据轮换/吊销界面、签名发布、自动升级、SQLite 状态迁移 |
| 数据源 | 文件、Docker 挂载清单、MySQL/PostgreSQL 逻辑导出 | Redis、应用级 Hook、数据库自动恢复、Docker 一致性编排 |
| 存储 | Restic 原生后端、S3 厂商预设、rclone 扩展、按服务器隔离路径、Secret 加密 | 保存前真实连通性探测、全部 Provider 的持续集成、不可变备份编排 |
| 调度与运行 | 5 段 Cron、IANA 时区、抖动、超时、完成宽限、RPO 迟到/超时推导、仓库串行、立即执行、幂等 Run | 错过任务后补跑；当前只支持 `missed_run_policy=skip` |
| 生命周期 | 数量/智能/GFS/时间保留、dry-run、Forget/Prune/Check 独立窗口、保护标签 | 自动恢复演练、应用级可恢复性证明、跨仓库复制 |
| 恢复 | 快照缓存、目录浏览、文件/目录恢复到全新隔离目录、禁止覆盖 | 跨 Agent 恢复、恢复目录 TTL/配额、生产路径受控回写 |
| 可观测性 | 运行统计、Agent 在线状态、计划倒计时、RPO 迟到/超时、备份失败/RPO 告警、9 类自定义通知渠道、按项目路由、去重、重复提醒、恢复通知、持久化重试、通知 SSRF 防护、操作审计 | Agent 离线/仓库 Check 失败告警、静默时段、多级升级、审计字段差异/外部归档 |
| 项目治理 | 公开源码、PolyForm Noncommercial 1.0.0、CI、Compose/systemd、安装和运维文档 | 非商业限制不符合 OSI 开源定义；商业使用需要另行授权 |
...
VaultMesh 是一个公开源码、自托管、多服务器的备份控制平台，采用 PolyForm Noncommercial 1.0.0 许可证供非商业用途使用。由于许可证限制商业用途，项目对外应使用 source-available（公开源码）描述，而不是 OSI Open Source。它负责：
...
### 3.1 MVP（P0）目标范围（非完成清单）

本节用于定义验收终点，不代表所有条目已经实现。当前状态以 0.2 节为准。

| 能力 | P0 说明 | 最低验收标准 |
|---|---|---|
| 用户与登录 | 单工作区、单管理员；密码 + TOTP 或 WebAuthn 通行密钥 | 密码安全存储；恢复码单次使用；会话可撤销；关键操作要求密码和已启用的第二因素 |
...
| 通知 | Webhook、Telegram、Email、Slack、Discord、企业微信、钉钉、Gotify、ntfy | 备份失败和 RPO 超时具备按项目路由、指纹去重、周期提醒、恢复通知与持久化重试；校验失败尚待接入 Incident |
...
- 随机抖动窗口，避免多主机同时上传；
- `missed_run_policy`：当前只实现 `skip`；`run_once` 是后续目标，必须先定义跨重启游标和幂等语义；
...
- Agent 只接受无凭据、无子路径/Query/Fragment 的控制面 Origin，且不跟随 HTTP 重定向，避免注册令牌或设备 Bearer 被转发；
...
- 状态变更接口使用 SameSite Cookie、精确 Origin，并强制 `application/json`、单 JSON 值、未知字段拒绝和请求体上限；恢复、删除等高风险操作要求近期认证；
- Web 与 API 独立部署；API 只允许显式配置的精确 Web Origin，不允许通配符 CORS；
- 密码登录、TOTP 登录和已认证会话内的密码/TOTP/恢复码确认已实现有界渐进限速；敏感操作按来源地址和当前会话隔离预算，生产反向代理仍需提供持久化或多副本共享限速；注册和恢复等入口继续按威胁模型评估独立限速；
- 所有对象访问检查工作区归属，不能只依赖前端隐藏；
- 高风险操作要求近期认证；
- 当前审计日志追加记录操作者、动作、目标引用、IP、HTTP 状态、时间和结果；后续补充字段前后差异、请求关联 ID 与外部防篡改归档；
- 严禁在错误响应中返回完整命令、环境变量或连接串。
- TOTP Secret 必须可逆加密，恢复码只保存摘要；验证器动态码拒绝同一时间步重放，待验证登录限制尝试次数；
- WebAuthn 用户句柄使用稳定随机值，Ceremony Session 只保存在服务端并由短时不透明 Cookie 关联；通行密钥要求可发现凭据和用户验证；
- Web 运行时 API 地址以纯文本数据加载而非环境变量插值 JavaScript，并拒绝 URL 凭据、Query、Fragment 和 HTTPS 页面降级到 HTTP API；
- 通知 HTTP/SMTP 目标在连接前解析并校验全部地址，不使用环境代理、不跟随重定向，默认拒绝私网/回环且始终拒绝链路本地和云元数据地址；自建服务必须显式开启私网访问；
...
| User | id, password_hash, webauthn_user_id, security_ciphertext, status | 管理员身份；TOTP 与通行密钥记录加密保存 |
...
项目管理页必须以 Server/Agent 作为一级分组，每台服务器下只展示自己的 Project，并保留“无项目”服务器作为明确的待配置状态。项目卡片再展示数据源、仓库渠道、Revision、执行计划与下次运行时间；跨服务器总量和风险仍在 Dashboard 聚合，避免把“集中管理”误做成扁平列表。

### 13.3 通知规则

默认不对每次成功发送通知，避免告警疲劳。当前实现以 `backup:<project_id>` 和 `rpo:<project_id>` 作为稳定指纹，发送首次 firing、用户设定周期的 repeat，以及可选 resolved。每条消息先写入 PostgreSQL Outbox，再由 Worker 最多尝试五次；通知失败不改变备份 Run 结果。

当前自动发送：

- 首次备份失败、`partial`、`timed_out`、`unknown` 或 `canceled`；
- 项目进入 `overdue`；
- 异常持续超过联系点重复周期；
- 上述状态恢复正常。
- 项目被暂停时关闭其 firing Incident，并按渠道设置发送一次恢复通知，避免对已停用项目持续重复告警。
...
详细渠道字段、路由、去重和安全边界见 `docs/NOTIFICATIONS.md`。
...
- **本地状态**：当前使用权限收敛的原子 JSON 文件保存设备身份、配置、Run 和待上报事件；规模化后再迁移到 SQLite（WAL）或等价嵌入式存储；
...
当前阶段判断：阶段 0 的代码闭环已经完成，真实云存储和恢复演练仍需持续执行；阶段 1 正在进行，RPO 推导、备份失败/RPO Incident、通知去重、恢复通知和持久化重试已落地，告警规则扩展、审计外部归档和发布工程是主要缺口。
...
| 技术难点 | 推荐实现方式 | 验证方法 |
|---|---|---|
| 中心离线仍需按时备份 | 期望配置版本下发；当前由原子 JSON 持久化设备身份、配置、Run 和 Outbox，规模化后迁移 SQLite | 关闭中心跨过多个计划时间点，再恢复并核对无丢失、无重复 |
...
Docker 自动发现、复杂告警升级树、多引擎和大屏展示，都应在这条闭环稳定后再增加。

---

## 19. 已接受决策与待决事项

当前代码已经体现以下决策，后续不应在没有 ADR 和迁移方案的情况下隐式改变：

1. 单工作区、单管理员起步，不直接做 SaaS 多租户；
2. Agent 首版面向 systemd 管理的 Linux 原生二进制；
3. 固定 Restic 执行引擎，存储能力以 Restic 原生后端和受控 rclone 扩展为边界；
4. 数据库当前只保证可验证的逻辑导出，自动恢复到新实例属于后续阶段；
5. Secret 由控制面使用主密钥加密托管，并只下发给项目所属 Agent；
6. P0 不承诺不可变备份，“保护快照”不等于对象存储 Object Lock；
7. 项目采用 PolyForm Noncommercial 1.0.0：允许许可证定义的非商业用途，商业使用需要另行获得授权。

仍需项目所有者明确决定并记录：

1. 商业授权的申请入口、价格模型和商用支持边界；
2. Agent 设备凭据的吊销、轮换和丢失恢复流程；
3. RPO 默认值、静默时段和告警升级策略；
4. 正式版本兼容承诺、数据库迁移策略和签名发布方式；
5. 生产环境推荐的不可变存储方案与责任边界。
...
9. PolyForm Noncommercial License 1.0.0：<https://polyformproject.org/licenses/noncommercial/1.0.0>
10. Open Source Initiative - Open Source Definition：<https://opensource.org/osd>
11. Grafana Alerting Contact points：<https://grafana.com/docs/grafana/latest/alerting/fundamentals/notifications/contact-points/>
12. Prometheus Alertmanager Configuration：<https://prometheus.io/docs/alerting/latest/configuration/>
13. Healthchecks Notification Integrations：<https://healthchecks.io/docs/configuring_notifications/>
14. Apprise：<https://github.com/caronc/apprise>

---

## 21. 最终项目说明

VaultMesh 是一个面向 Linux VPS、Homelab 和小型技术团队的公开源码、自托管备份控制平台，采用 PolyForm Noncommercial 1.0.0 供非商业用途使用。平台通过轻量 Agent 管理多服务器的声明式备份策略，使用 Restic 将客户端加密的数据从源服务器直接写入用户自己的存储，并集中管理运行状态、计划、快照、保留、校验和受控恢复。RPO 违约计算、备份失败/RPO 告警、去重、恢复通知和持久化重试已纳入当前实现；自动恢复演练仍是后续能力。
...
当前实现聚焦 Linux、Restic、文件、Docker 挂载、MySQL/PostgreSQL 逻辑导出和多类 Restic 仓库，已经打通“配置—执行—上报—快照—保留/校验—RPO/通知—隔离恢复”的纵向链路。下一阶段优先完成真实 Provider 集成测试、扩展告警事件、控制面灾备、恢复演练和发布安全；Redis、数据库自动恢复、不可变存储增强、多管理员权限和多备份引擎继续按阶段评估。
