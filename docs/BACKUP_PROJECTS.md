...
| 层 | 当前字段 | Agent 行为 |
|---|---|---|
| 数据源 | files、Docker、MySQL、PostgreSQL、必需/可选 | 文件直接读取；Docker 生成脱敏清单并解析挂载；数据库先生成受保护的逻辑导出 |
...
`schedule.jitter_seconds` 只作用于 Cron 触发的计划备份，用于避免大量 Agent 同时访问仓库。立即备份、清理预览、快照同步/浏览/保护/恢复以及独立维护窗口不会继承该随机延迟，用户操作和维护任务会在取得仓库锁后立即执行。
...
每个来源都通过 `required` 明确失败策略，默认应保持为 `true`：

- 必需来源准备失败时，Agent 不会调用 Restic backup，整次运行标记为 `failed`；
- 可选来源准备失败时，Agent 跳过该来源并继续备份其余内容；只要仍有内容可备份，就会创建快照并将运行标记为 `partial`；
- `stats.optional_sources_skipped` 记录被跳过来源的 ID、类型和脱敏错误，便于告警和排障；
- 如果所有来源都无法准备，即使它们均为可选，运行仍会失败，不会创建空快照。

数据库、配置目录等恢复所必需的内容不应设为可选。可选模式更适合临时挂载、非关键 Docker 容器或允许短时不可用的附加目录。
