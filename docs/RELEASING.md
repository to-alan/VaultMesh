...
1. 更新 CHANGELOG 的 Unreleased，整理该版本变更、修复、已知限制、数据库/配置/Agent unit 迁移说明。
   新端口流程的部署包需包含 `INSTALLER_API=2`，并验证无域名本地入口、3000 占用自动选端口、Nginx 接入及 `configure-proxy` 保留密码/数据库；旧 RC1 不通过替换附件获得这些功能。
...
- 将版本说明提交为 `docs/RELEASE-vX.Y.Z.md`（候选版同理）；工作流会在公开前将此文件写入 Draft 的发布说明。说明中的文档链接使用该版本标签的完整 GitHub URL，确保 Release 页面可直接访问。
