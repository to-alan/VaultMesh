...
后端只接受精确的 provider、环境变量与 Restic option 白名单。不会接受任意环境变量、`sftp.command`、`rclone.program` 等可执行命令型参数，避免仓库配置变成 Agent 上的命令执行入口。当前“校验并预览”只验证字段与 URL 结构；真实连通性由对应 Agent 在首次 `restic snapshots` / `restic init` 时验证。
