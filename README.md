...
管理员首次启动时由环境变量引导创建，随后密码哈希和安全资料持久化到 PostgreSQL；再次修改 `.env` 不会覆盖数据库中的现有密码。可在“个人中心”修改密码、启用 TOTP 二步验证、生成一次性恢复码并注册通行密钥。修改密码或停用二步验证会撤销全部会话。登录后浏览器使用不可由 JavaScript 读取的 HttpOnly 会话 Cookie，前端不保存管理员认证凭据；当前会话最长 12 小时，Control Plane 重启后需要重新登录。
