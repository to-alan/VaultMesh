...
- 项目暂停/恢复、Restic 原生扫描边界、最多 N 份/智能/GFS/按时间保留、真实仓库 dry-run 清理预览，以及 Forget/Prune/Check 独立维护窗口；
- 快照索引、按目录浏览、永久保护标签，以及强制写入 Agent 隔离目录且禁止覆盖的文件级/目录级恢复；
- 带租约和幂等键的“立即备份”命令；
...
项目边界、当前实现与路线图见 [VaultMesh 项目说明书](./VaultMesh-项目说明书.md)。备份项目字段、执行顺序及依据见 [备份项目策略说明](./docs/BACKUP_PROJECTS.md)，恢复流程见 [快照浏览与安全恢复](./docs/SNAPSHOT_RECOVERY.md)，升级、控制面灾备和故障处理见 [运维手册](./docs/OPERATIONS.md)。
...
安装结束后务必备份 `/opt/vaultmesh/.env` 和 PostgreSQL。前者包含解密 Secret 所必需的主密钥；只备份数据库并不能完整恢复控制面。可直接使用 [运维手册中的控制面备份步骤](./docs/OPERATIONS.md#控制面备份)。
...
- Restic 0.17.0 或更高版本（安全恢复依赖 `restore --overwrite never`）；
...
注册成功后，从环境文件中删除 `VAULTMESH_ENROLLMENT_TOKEN` 并重启服务。设备凭据保存在 `/var/lib/vaultmesh-agent/state.json`，权限为 `0600`。恢复任务默认写入 `/var/lib/vaultmesh-agent/restores/<command-id>`；完成验收和取回文件后由管理员清理该目录，VaultMesh 不会自动删除恢复产物。
...
测试覆盖控制面纵向 API、管理员密码持久化与会话撤销、TOTP/一次性恢复码、全局仓库迁移、一次性 Agent 注册、Secret 加解密、Agent 状态恢复、项目暂停/恢复、幂等 Run、Restic 成功/部分成功解析、保留与校验命令、快照索引/浏览/保护/禁止覆盖恢复、数据库暂存产物和 Docker 挂载清单脱敏。CI 额外执行 Go 竞态检测、`go vet`、Govulncheck、npm 高危漏洞审计，并构建 Control/Web/Agent 三个镜像。WebAuthn 的协议校验由 Go WebAuthn 库完成，真实设备注册仍应在目标 HTTPS 域名上做一次验收。
...
- 仓库创建目前校验字段和 URL 结构，但尚未在保存前执行真实连通性测试；首次 Restic 操作才会验证远端凭据和权限；
- 调度器当前只支持错过执行后跳过（`missed_run_policy=skip`），尚未实现恢复在线后补跑一次；
- Dashboard 已提供运行统计和下次任务倒计时，但 RPO 违约计算、通知投递和恢复通知尚未实现；
- 已支持人工快照浏览和隔离恢复，但还没有跨仓库复制、自动恢复演练、恢复产物自动过期清理与原路径一键回写；
- 直接 S3 模式不提供不可变备份保证；
- Docker 挂载卷默认为崩溃一致性快照，不会自动停止容器；数据库容器应同时配置 MySQL/PostgreSQL 逻辑备份；
- Agent 当前本地状态使用受权限保护的原子 JSON 文件，后续规模化时可迁移到 SQLite；
- 尚未发布稳定版本和自动更新通道；
- 仓库目前没有 `LICENSE` 文件，因此是公开源码但还不是可自由使用、修改和分发的开源发行版；许可证需要由项目所有者明确选择。
